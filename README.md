...
# ⚾ MLB 混合式 AI 投球分析系統 — 架構與對接手冊

本系統採用模組化的管線設計 (Pipeline)，涵蓋從原始數據過濾、建立黃金基準、計算進壘信心度，到生成教練級修正建議的完整流程。以下為核心分析模組 (Phase 1 ~ 4) 與總司令部 (`run_hybrid_ai_system`) 的詳細資料流與對接規格。

---

## 🧩 核心分析模組 (Phase 1 ~ Phase 4)

### 🟢 Phase 1: 建立黃金基準池
利用歷史大數據，鎖定成功落點，並透過 KNN 演算法找出與目標投手「身材條件最相似」的歷史投球軌跡。

* **函式名稱：** `build_success_similarity_pool`
* **接收參數 (Inputs)：**
  * `df` *(pd.DataFrame)*: 清理過後的歷史投球資料庫 (`df_clean`)。
  * `target_pitch_type` *(str)*: 目標球種（例如 `'SL'`）。
  * `target_zone` *(int)*: 預期落點的九宮格網格（1~9）。
  * `target_pitcher` *(dict)*: 目標投手的物理特徵（需包含 `release_pos_x`, `release_pos_z`, `release_extension`）。
  * `top_k` *(int)*: 欲萃取的基準球數量（預設 100）。
* **回傳型態 (Returns)：** * *(pd.DataFrame)*: 包含篩選出之基準球的 DataFrame，並新增 `physical_distance` 欄位。

### 🟢 Phase 2: 萃取靶心與共變異數矩陣
接收 Phase 1 的黃金基準池，計算出多維空間中的「完美靶心」與具備物理代償特性的「容錯地形圖」。

* **函式名稱：** `run_phase2_analysis`
* **接收參數 (Inputs)：**
  * `pool_df` *(pd.DataFrame)*: Phase 1 回傳的黃金基準池。
  * `optimization_features` *(list[str])*: 欲進行最佳化之特徵清單（預設為球速、轉速、轉軸等）。
* **回傳型態 (Returns)：** * `target_mean` *(np.ndarray)*: 目標特徵的平均值（靶心向量）。
  * `target_covariance` *(np.ndarray)*: 目標特徵的共變異數矩陣。

### 🟢 Phase 3: 計算馬氏距離信心度
計算當前失投球與完美靶心之間的「馬氏距離平方」，考量物理特徵間的合理代償，最後轉換為進壘信心度。

* **函式名稱：** `evaluate_pitch_confidence`
* **接收參數 (Inputs)：**
  * `current_pitch` *(dict)*: 當前投球的原始物理數據。
  * `target_mean` *(np.ndarray)*: Phase 2 算出的靶心向量。
  * `target_covariance` *(np.ndarray)*: Phase 2 算出的共變異數矩陣。
  * `optimization_features` *(list[str])*: 計算使用之特徵清單。
* **回傳型態 (Returns)：** * `confidence_score` *(float)*: 落入目標位置的預測信心度 (0% ~ 100%)。
  * `mahalanobis_dist` *(float)*: 原始馬氏距離數值。
  * `deltas_dict` *(dict)*: 各特徵與靶心的純量誤差字典。

### 🟢 Phase 4: 加權梯度下降教練引擎
結合人體工學難度權重（Mobility Weights），在標準化後的馬氏空間中執行梯度下降。系統會自動繞過「難改的特徵」，尋找一條阻力最小的捷徑。

* **函式名稱：** `gradient_descent_coach`
* **接收參數 (Inputs)：**
  * `current_pitch` *(dict)*: 當前投球數據。
  * `target_mean` *(np.ndarray)*: Phase 2 靶心向量。
  * `target_cov` *(np.ndarray)*: Phase 2 共變異數矩陣。
  * `features` *(list[str])*: 優化特徵清單。
  * `target_confidence` *(float)*: 目標及格信心度（預設 85.0）。
* **回傳型態 (Returns)：** * `advice_dict` *(dict)*: 給予投手的具體物理修正量建議。
  * `current_conf` *(float)*: 若照建議修正後，預期達到的目標信心度。

---

## 🚀 系統統整總管線 (Pipeline)

### `run_hybrid_ai_system`
本函式為整個專案的總司令部，負責將外部模型的預測結果與系統內部的四大 Phase 進行串接，實現真正的 End-to-End 分析流程。

#### ⚙️ 執行流程與計算原理
1. **球種辨識：** 呼叫外部 ML 模組 `predict_pitch()`，在無標籤狀況下判定輸入數據的球種。
2. **球威評估：** 基於歷史數據庫與動態特徵權重（`PITCH_CONFIG`），計算該球軌跡品質的綜合 PR 值 (Stuff+)。
3. **控球分析與教練建議：**
   * 啟動 Phase 1 建立該球種與該投手的客製化黃金池。
   * 啟動 Phase 2 萃取靶心與矩陣。
   * 啟動 Phase 3 結算進壘信心度與具體誤差。
   * 啟動 Phase 4 執行最佳化引擎，產出阻力最小的修正路徑。

#### 📥 接收參數 (Inputs)
* `raw_pitch_data` *(dict)*: 雷達測速槍產出的單顆球原始物理數據。
* `target_zone` *(int)*: 教練/投手預定投擲的九宮格目標。
* `pitcher_profile` *(dict)*: 該投手之身體條件輪廓。
* `df_database` *(pd.DataFrame)*: 系統預先載入且清洗完畢的歷史資料庫。

#### 📤 回傳資料型態 (Returns)
回傳一份包含完整戰情報告的 Python 字典 (`dict`)，可直接供前端或終端機渲染：
